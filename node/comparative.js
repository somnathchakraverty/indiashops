...
var dealsOnMobile = function( product, result, callback ){
							...
							async.parallel([
									function(callback) {
										async.waterfall([
												...
												{
													dealsOnMobile(product, result, callback)
												}
											],
											function(err,data) {
												main_callback();
...
/*var productCompare = function( product, callback ){
 ...
 params['category_id'] 	= product.product_detail.category_id
 params['saleprice'] 	= product.product_detail.saleprice
 params['id'] 			= product.product_detail.id
 params['similar'] 		= 1

 url = composer_url + "compare_products.php?"+querystring.stringify(params);
 request(url, 'compare', callback);
 }*/