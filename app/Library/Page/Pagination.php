<?php namespace indiashopps\Library\Page;
...
class Pagination
{
    ...
    public function getRelPrevNextLinkTags()
    {
        ...
        $prev = str_replace('{page?}', $currentPage - 1, $href);
        $prev = str_replace('{page}', $currentPage - 1, $prev);
        $next = str_replace('{page?}', $currentPage + 1, $href);
        $next = str_replace('{page}', $currentPage + 1, $next);
        // Store em
        $prevNextTags = array(
            '<link rel="prev" href="' . (trim($prev,"-")) . '" />'
        );
        // There is a page after this one
        if ($numberOfPages > $currentPage) {
            array_push(
                $prevNextTags,
                '<link rel="next" href="' . (trim($next,"-")) . '" />'
            );
        }
        return $prevNextTags;
    }
     ...
     * @return void
     */
    public function parse($ajax = false)
    {
        $data['ajax'] = ($ajax) ? '-ajax' : '';
        ...
        $_response = (string)view("v2.pagination.render", $data);
