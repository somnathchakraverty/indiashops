...
var CONTENT = {
    ...
    append: function (content, section) {
        ...
        var callback = section + "_callback";
    ...
    refresh: function (section) {
        ...
        if (el.is(':empty')) {
            ...
            el.find('.carousel').each(function (i, e) {
                var items = $(e).attr('data-items');
                var scroll = $(e).attr('data-scroll');
...
$(document).ready(function () {
    ...
    $(document).on('click', '.coupon_thum', function (e) {
        ...
        var in_url = $(this).attr('inpage');
        var out_url = $(this).attr('outurl');
        var code = $(this).attr('code');

        if (typeof type != 'undefined' && type == "coupon") {
            setCookie('coupon_code', code, 1);
        }

        window.open(in_url);
        window.location = out_url;
    ...
    $(document).on('submit', '#main_search', function (e) {
        ...
        if (action.indexOf('coupon') > -1)
            var search_url = action + "?search_text=" + create_slug(search_text);
        else
            var search_url = action + "/" + cat_id + "/" + create_slug(search_text);
