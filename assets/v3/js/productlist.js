...
var ListingPage =
{
    ...
    view: {
        ...
        renderProducts: function (products) {
            ...
            if (products.indexOf("thumnail2") > -1) {
                this.vars.p_wrapper.html(products).hide().fadeIn('slow');
                $('[data-toggle="tooltip"]').tooltip();

                if ($(products).find('.thumnail2').length < 32) {
    ...
    controller: {
        ...
        getNextPage: function () {
            ...
            this.getProducts(true, function (products, facet) {
                ...
                ListingPage.controller.updateLoadMoreContent(facet);
        ...
        updateLoadMoreContent: function (facet) {
            ...
            var total_count = from + total;

            if (typeof facet != "undefined" && typeof facet.total != "undefined") {
                if (facet.total < total_count) {
                    total_count = facet.total;
                }
            }

            var range = String(from + 1) + " - " + String(total_count);
            ...
            if (that.localCache.exist(filter_url)) {
                ...
                $.get(filter_url, function (data) {
                    ...
                    try {
                        ...
                        if (send) {
                            if ($.isFunction(callback)) callback(data.products, data.facet);
                        }
                        else {
                            that.functions.overlay.hide();
        ...
        functions: {
            ...
            'overlay': {
                ...
                'show': function () {
                    ...
                    $('html, body').animate({
                        scrollTop: $("#right_container").offset().top
                    }, 800);
