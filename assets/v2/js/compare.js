...
$(document).on('change', '#product_wrapper .add-to-compare', function(){
    compare.fadeIn();
    var this_product = $(this).closest(".probox");
    ...
    if( $(this).is(":checked") )
    {
        if (imgtodrag && update_compare_cookie( prod_id ) )
        {
            ...
            imgclone.animate({'width': 0, 'height': 0 }, function (){
                ...
                data['title'] 	= this_product.find("h4 > a").text();
                data['url'] 	= this_product.find("h4 > a").attr('href');
                data['price']	= this_product.find(".pricepro").text().replace("Rs.","");
