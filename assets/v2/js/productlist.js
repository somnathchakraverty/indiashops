...
var ListingPage =
{
	...
	view : {
		...
		updateFilterCheckbox: function( attr, values )
		{
			...
			$.each(values,function(key,v){
				...
				try
				{
					element = $(".checkbox."+attr+" #chk"+clean(v.key));
					element.prop("disabled",false);
					element.closest(".fix-slide-checkbox").find("span.count").html("["+v.doc_count+"]")
					element.closest(".checkbox."+attr).detach().prependTo($("#"+attr+"-wrapper .nano-content"));
				}
				catch(e)
				{
					console.log(attr)
		...
		defaultFilterUpdate: function(){
			...
			$.each( fields, function(attrib,val){
				...
				$.each(values,function(k,value){
					element = $("."+attrib+" #chk"+clean(value) );
	...
	controller: {
		...
		getNextPage : function(){
			...
			/*this.getProducts( false, function(products){
		...
		functions: {
	        ...
	        'overlay' 	: {
	        	...
	        	'show'	: function(){
	        		...
	        		$('html, body').animate({
						scrollTop: $("#slide-banner").offset().top
					}, 800);
