	...
	"controller" : {
		...
		"init" 	: function()
		{
			...
			$(document).on("click",".single-prop",function(){
				var id = $(this).attr("data-groupname").split(",");
		...
		"applyPreviousFilter" : function( callback )
		{
			...
			var filtered = false;
			var ready = true; var i=0;var interval;
	...
	"view"		 : {
		...
		"render" : function( $data )
		{
			try
			{
				...
				if( result.coupons.indexOf("product-listing") > -1 )
				{
		...
		"update" : function( $data )
		{
			try
			{
				...
				loading 	= false;
                CouponListing.controller.overlay.hide();
			}
			catch( err )
			{
				return false;
