...
.product-slider li img,.product-slider .slick-dots button img{max-width:280px;max-height:280px;margin:auto;padding-top:5px;}
...
.product-slider .slick-dots li button img{max-width:38px;max-height:38px;border:0px;padding-top:0px;}
...
.nbs-flexisel-item{float:left;margin-left:0px;padding:0;cursor:pointer;position:relative;line-height:0}
...
.detailsproductname{padding:0px;text-align:center}
.detailsproductname h1{font-size:18px;color:#000;text-align:center;}
.vendorlogomobile{width:100px; height:37px; border:1px solid #dedede; margin:10px auto; text-align:center; padding:3px;}
.btn-success{font-size:16px!important;margin:10px auto!important;display:block!important;font-weight:700;outline:0!important;text-shadow:rgba(62,62,62,.75) 0 0 5px;}
.detailsproductname{padding:0;text-align:center}
...
.bgcolorproduct li label{padding-right:10px;color:#e40046;}
</style>
...
<div class="fullbgpro" style="margin:10px 0px 20px 0px;padding-bottom:15px;">
    <div class="container">
        ...
        <div class="slick-thumbs">
            <ul>
                @foreach( $images as $key => $image )
                    <li class="{{( $key ==0 ) ? 'active' : ''}}"><a data-target="#pic-{{$key}}" data-toggle="tab"> <img
                                    src="{{getImageNew($image, 'XS')}}"/> </a></li>
                @endforeach
            </ul>
        </div>        
        @if(isset($product->product_url))
            <div class="row">
                <div class="col-xs-6">
                    <div class="vendorlogomobile"><a href="{{$product->product_url}}" target="_blank"
                                                     onClick="ga('send', 'event', 'shop', 'click_mobile');"
                                                     rel="nofollow"> <img
                                    src="{{config('vendor.vend_logo.'.$product->vendor )}}"
                                    alt="{{config('vendor.name.'.$product->vendor )}} Image" class="img-responsive">
                        </a></div>
                </div>
                <div class="col-xs-6"><a href="{{$product->product_url}}" target="_blank"
                                         onClick="ga('send', 'event', 'shop', 'click_mobile');" rel="nofollow"
                                         class="btn-success" style="margin-top:10px!important;font-size:13px!important;padding:10px 6px;">Go to Store</a></div>
            </div>
        @endif </div>        
...
<div class="container">
<div class="shipping"><b>FREE</b> Shipping . EMI . COD</div></div>
</div>

@if((isset($detail_meta) && isset($detail_meta->meta)) || !empty($data->product_detail->description))
    <div class="fullbgpro">
        <div class="fullwidth padding10">
            <div class="bgcolorproduct">
                <h3 class="haddingdetails">Description</h3>
...
@if( isset( $slider[0] ) && !empty($slider[0]) )
    <?php $image = $slider[0]; ?>
    <div class="container"><a href="{{$image->refer_url}}" target="_blank"> <img class="img-responsive adimg"
                                                                                 src="{{$image->image_url}}"
                                                                                 alt="{{$image->alt}}"> </a></div>
@endif
<!--END-PART-2-->
...
<div class="fullbgpro">
    <div class="container" id="by-brand-wrapper">
...
@if( isset( $slider[1] ) && !empty($slider[1]) )
    <?php $image = $slider[1]; ?>
    <div class="container"><a href="{{$image->refer_url}}" target="_blank"> <img class="img-responsive adimg" src="{{$image->image_url}}" alt="{{$image->alt}}"> </a></div>
@endif
...
@if(isset($similar) && is_array($similar) && count($similar) > 0)
            ...
            <div class="fullwidth">
                <ul id="flexiselDemo1">
                    @foreach( $similar as $sim )
                        ...
                        <li>
                            <div class="thumnail">
                                <div class="productboxallcat"><a href="{{product_url($product,$book)}}"> <img
                                                class="productallcatimg" src="{{getImageNew($product->image_url, 'M')}}"
                                                alt=""> </a></div>
                                <div class="productname"><a
                                            href="{{product_url($product,$book)}}"> {{$product->name}} </a></div>
                                <div class="star-ratingreviews">
                                    <div class="star-ratings-sprite"><span style="width:52%"
                                                                           class="star-ratings-sprite-rating"></span>
                                    </div>
                                </div>
                                <a href="{{product_url($product,$book)}}" class="price">
    ...
    @if( isset( $slider[3] ) && !empty($slider[3]) )
        <?php $image = $slider[3]; ?>
        <div class="container"><a href="{{$image->refer_url}}" target="_blank"> <img style="padding-bottom:10px;" class="img-responsive adimg" src="{{$image->image_url}}"  alt="{{$image->alt}}"> </a></div>
    @endif
    <!--END-PART-4-->
    <script>
