...
@section('page_content')
    <style>
        ...
        #details table tr td { border: 1px solid #c7c7c7; padding: 5px; }
        #details table tr td p { font-size: 12.5px; text-align: left; }
                        ...
                        @if($main_product->saleprice>0)
                            <div class="lastprice">
                                ...
                                @else
                                    best price
                                @endif
                                <span id="s_price1">Rs {{number_format($main_product->saleprice)}}</span>
                            </div>
                        @endif
                        @if(isComparativeProduct($product))
    ...
    @if( isComingSoon($product) && isset($product->release_date) )
        <div class="whitecolorbg">
            <div class="container">
                ...
                <span class="lastpricevanderbutton" style="margin-top: 3px;">{{$product->release_date}}</span>
            </div>
        </div>
    @endif
    <!--THE-PART-2-->
    ...
    <!--END-PART-3-->
    @if(app('seo')->getExcerpt() || ( isset($main_product->excerpt) && !empty($main_product->excerpt)) )
        <section id="excerpt">
            <div class="whitecolorbg padding15px">
                <div class="container">
                    <div class="highlightspoint more_content_wrapper">

                        <div class="more-less-product1 small">
                            <p>
                                @if(app('seo')->getExcerpt())
                                    {!! app('seo')->getExcerpt() !!}
                                @elseif(isset($main_product->excerpt))
                                    {!! $main_product->excerpt !!}
                                @endif
                            </p>
                        </div>
                        <a href="javascript:void(0)" class="moreproduct">Read More <span>&rsaquo;</span></a>
    ...
    @if(isComparativeProduct($product))
        <section id="mobile_reviews">
            @include('v3.mobile.product.reviews')
        </section>
    @endif
...
@section('scripts')
    ...
    <script>
        ...
        document.addEventListener('jquery_loaded', function (e) {
            ...
            $(window).scroll(function (event) {
                if(flag==0){
                    <?php if(!is_array($product->vendor) && ($product->vendor > 0) && in_array($product->vendor,config('vendor_update_ids'))){
                    ?>
                    var myData = {
                        ...
                        success: function (responce) {
                            ...
                            if (obj != '') {
                                if (typeof obj.saleprice !== 'undefined') {
                                    $('#s_price1').html("Rs "+ numberWithCommas(obj.saleprice));
                                }
                                if (obj.track_stock == 1) {
                                    $('#t_stock1').html('<a href="{{(isset($affLink)) ? $affLink : "" }}" class="lastpricevanderbutton" style="width:200px">BUY FROM {{config('vendor.name.'.$product->vendor)}}
                                    <span class="right-arrow" style="margin-top:1px;"></span> </a>');
                                }
                                else {
                                    $('#t_stock1').html('<a href="javascript:void(0)" class="lastpricevanderbutton" style="width:200px">OUT OF STOCK </a>');
                                }
