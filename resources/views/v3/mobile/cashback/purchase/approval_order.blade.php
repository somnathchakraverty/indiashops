<div style="font-size:14px;font-family:Verdana, Arial, Helvetica, sans-serif, circular, 'Circular Std Black';">
    ...
    You have received a request for Approval below order from : {{ucwords($user->name)}}<br/><br/>
    Click <a href="{{$order_url}}">HERE</a> to check the details..<br/>
                            ...
                            @foreach($order->products as $product)
                                <tr class="border-bottom">
                                    ...
                                    <td style="text-align:center;">
                                        Rs {{number_format($product->product_price * $product->quantity)}}</td>
                                    <td style="text-align:center;">
                                        Rs {{number_format($product->cashback * $product->quantity, 2)}}</td>
