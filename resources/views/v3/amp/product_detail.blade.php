...
if (!is_array($image)) {
    $img[0] = getImageNew($product->image_url, $product->vendor);
} else {
    $img = $image;
}
...
<head>
    ...
    <?php endif; ?>
    <?php endif; ?>
    <link rel="canonical" href="{{route('product_detail_non',[$product->grp, create_slug($product->name),$product->id, $product->vendor])}}"/>
    ...
    <style amp-custom>
        ...
        .description-list { width: 100%; margin: 0px; text-align: left; overflow-y: scroll; overflow-x: none; }
        ...
        .descriptionbox { max-width: 300px; margin: auto auto 40px; }
    ...
    <div class="aboutuspartbottmob">
        <div class="slidesbox">
            <amp-carousel width="300" height="280" type="slides">
                ...
                { ?>
                <a href="#">
                    <amp-img class="product_img" src="<?=getImageNew($image,
                            'M')?>" title="<?php echo clean($product->name);?>" width="122" height="250" alt="<?php echo clean($product->name);?>"/>
                </a>
                <?php
                }
                }
...
<section id="contactus" class="overlay-contactus">
    <div class="container">
        ...
        <div class="product-detail-price">
            <ul>
                <li><a href="#"><i class="fa fa-star-o starcolor" aria-hidden="true"></i>
                        <i class="fa fa-star-o starcolor" aria-hidden="true"></i>
                        <i class="fa fa-star-o starcolor" aria-hidden="true"></i>
                        <i class="fa fa-star-o" aria-hidden="true"></i> <i class="fa fa-star-o" aria-hidden="true"></i></a>
                </li>
            </ul>
        </div>
        ...
        @if(isset($product->description) && !empty($product->description))
            <div class="">
                ...
                <div class="description-list">
                    <ul>
                        {!! amp_desc_table_to_div($product->description) !!}
                    </ul>
                </div>
            </div>
        @endif
...
<div class="allcategories">
    ...
    <amp-carousel width="400" height="350" layout="responsive" type="slides">
        ...
        if ($viewpro->_index == "books") $target_url = newUrl('product/' . create_slug($viewpro2->name . " book") . "/" . $viewpro->_id); else
            $target_url = newUrl('product/' . create_slug($viewpro2->name) . "/" . $viewpro->_id);

        if ($viewpro2->vendor == 0) $target_url = newUrl('product/' . create_slug($viewpro2->name) . "/" . $viewpro->_id);
        ?>
        <div class="viewedbox">
            <div class="mobilenamecat">
                <a href="<?=$target_url?>" title="<?php echo clean($viewpro2->name); ?>">
                    <amp-img class="product_img" src="<?=getImageNew($viewpro2->image_url, $viewpro2->vendor,
                            'M')?>" width="108" height="180" alt="<?php echo clean($viewpro2->name); ?>"/>
                </a>
            </div>
