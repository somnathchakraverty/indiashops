...
@section('page_content')
                ...
                <div class="productimgdetailsbox">
                    ...
                    @if(hasKey($product,'variants') && count($product->variants) > 0)
                        <div class="productpricelast">
                            <div class="lastprice">
                                Storgae, RAM
                                <select id="variants" class="form-control fildname" on="change:AMP.navigateTo(url=event.value)">
                                    @foreach($product->variants as $key => $variant)
                                        <option value="{{route('amp_detail_comp',[cs($variant->name),$variant->id])}}" {{($variant->id == $product->id) ? 'selected' : ''}}>{{$key}}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                        ...
                        <table>
                            ...
                            @if($brand_count >= 5)
                                <tr>
                                    ...
                                    <td>
                                        <a href="{{route('amp.brands.listing',[cs($product->grp), cs($product->brand),cs($product->category)])}}">
    ...
    @if(isset($youtube_url))
        <section id="videos_sec">
            ...
            <div class="whitecolorbg tabtop40">
            <div class="container">
                    ...
                    <amp-carousel class="full-bottom" height="350" layout="fixed-height" type="carousel">
                        @foreach($compare_products as $product)
                            @php
                                ...
                                if(isset($main_product) && !empty($main_product) && isComparativeProduct($main_product))
                                {
                            ...
                            @endphp
                            @include('v3.common.product.amp.card', [ 'compare_url' => $compare_url ])
                        @endforeach
                    </amp-carousel>
                </div>
            </div>
            ...
            <div class="whitecolorbg">
                ...
                <h2>Reviews and latest news</h2>
                    <amp-selector role="tablist" layout="container" class="ampTabContainer">
                        <div role="tab" class="tabButton tableft15" selected option="a">User Review</div>
