...
<?php foreach( $navigation as $key => $nav ): ?>
    <div id="ver_pt_menu<?=$nav['category']->id?>" class="pt_menu">
        <div class="parentMenu">
            ...
            <a class="sprite_cat" href="<?=$parent_url?>" title="<?=$nav['category']->name?>"><span class="cate-thumb sprite <?= strtolower(str_replace(" & ","-",$nav['category']->name)); ?>">
                </span>
                <?=$nav['category']->name?>
            </a>
        </div>
                                ...
                                <div class="itemSubMenu level3">
                                    <div class="itemMenu level4">
                                        ...
                                        <?php if( $cchilds > 0 ): $j = 1; ?>
                                            ...
                                            ?>
                                            <?php endforeach; ?>
                                            <a class="itemMenuName level4 view" href="<?=$child_url?>" title="View All <?=$cnav['category']->name?>">
                                                <span>View All</span>
                                            </a>
                                        <?php endif;?>
                                    </div>
    ...
    <div id="ver_pt_menu20" class="pt_menu noSub">
        <div class="parentMenu">
            <a class="sprite_cat" href="<?=newUrl('coupons')?>">
                <span class="cate-thumb sprite coupons"></span>
				Coupons
            </a>
        </div>
    </div>
    <div id="ver_pt_menu19" class="pt_menu noSub">
        <div class="parentMenu">
            <a class="sprite_cat" href="<?=newUrl('all-categories')?>">
                <span class="cate-thumb sprite view_all"></span>
                    View All
            </a>
        </div>
    </div>